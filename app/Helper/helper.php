...
if (!function_exists('settingsKeys')) {
    function settingsKeys()
    {
        return $settingsKeys = [
            ...
            'FROM_EMAIL' => "",
            'FROM_NAME' => "",
            "measurement_number_prefix" => "#MSR-000",
            "customer_number_prefix" => "#CUST-000",
...
if (!function_exists('measurementNumber')) {
    function measurementNumber()
    {
        $latestMeasurement = Measurement::where('parent_id', parentId())->latest()->first();
...
if (!function_exists('SetLimit')) {
    function SetLimit($subscription, $id)
    {
        ...
        $users = User::where('parent_id', '=', $id)->whereNotIn('type', ['super admin', 'owner',])->update(['is_active' => 0]);
        $users = User::where('parent_id', '=', $id)->whereNotIn('type', ['super admin', 'owner', 'customer', 'employee'])->take($user_limit)->get()->each(function ($user) {
        ...
        $customer = User::where('parent_id', '=', $id)->whereIn('type', ['employee'])->update(['is_active' => 0]);
        $customer = User::where('parent_id', '=', $id)->whereIn('type', ['employee'])->take($employee_limit)->get()->each(function ($customer) {
            $customer->update(['is_active' => 1]);
        });
...
if (!function_exists('measurementPrefix')) {
    function measurementPrefix()
    {
        $settings = settings();
        return $settings["measurement_number_prefix"];
    }
}
if (!function_exists('orderPrefix')) {
    function orderPrefix()
    {
        $settings = settings();
        return $settings["order_number_prefix"];
    }
}
...
if (!function_exists('getTax')) {
    function getTax($itemtax)
    {
        $itemtaxes = \App\Models\Tax::whereIn('id', explode(',', $itemtax))->get();
        return $itemtaxes;
    }
}
...
if (!function_exists('taxRate')) {
    function taxRate($rate, $amount, $quantity)
    {
        return ($rate / 100) * ($amount * $quantity);
    }
}
    ...
    function defaultTemplateList()
    {
        return [
            ...
            'customer_create' => [
                ...
                'name' => 'New Customer',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}'],
                'subject' => 'Welcome to {company_name}!',
                'templete' => '<p><strong>Dear {customer_name},</strong> Welcome to <strong>{company_name}</strong>! We are thrilled to have you with us and look forward to providing an exceptional experience. If you have any questions or need support, feel free to contact us at:</p><ul><li>Email: <a href="mailto:{company_email}">{company_email}</a></li><li>Phone: {company_phone_number}</li></ul><p>Thank you for choosing <strong>{company_name}</strong>! <strong>{company_name}</strong><br>{company_address}<br>{company_currency}</p><p>&nbsp;</p>',
                'sms_message' => 'Hi {customer_name}, welcome to {company_name}! Reach us at {company_email} or {company_phone_number}. Thanks for joining!',
            ],
            'measurement_create' => [
                'module' => 'measurement_create',
                'name' => 'New Measurement',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{measurement_number}', '{date}', '{cloth_type}', '{responsible_user_name}', '{responsible_user_email}', '{responsible_user_phone}', '{measurement_details}'],
                'subject' => 'Measurement Confirmation - {measurement_number}',
                'templete' => '<p><strong>Dear {customer_name},</strong></p><p>Thank you for choosing <strong>{company_name}</strong>! We are pleased to confirm that your measurement has been successfully recorded. Below are the details of your measurement:</p><ul><li><strong>Measurement Number:</strong> {measurement_number}</li><li><strong>Date:</strong> {date}</li><li><strong>Cloth Type:</strong> {cloth_type}</li><li><strong>Measurement Details:</strong>&nbsp;</li></ul><p>{measurement_details}</p><p>If you have any questions or require further assistance, feel free to contact our representative:</p><ul><li><strong>Responsible User:</strong> {responsible_user_name}</li><li><strong>Email:</strong> <a href="mailto:{responsible_user_email}">{responsible_user_email}</a></li><li><strong>Phone:</strong> {responsible_user_phone}</li></ul><p>We look forward to serving you again!</p><p><strong>{company_name}</strong><br>{company_address}<br>{company_currency}</p>',
                'sms_message' => 'Hi {customer_name}, your measurement {measurement_number} on {date} has been recorded at {company_name}. Cloth: {cloth_type}.'
            ],
            'measurement_assign' => [
                'module' => 'measurement_assign',
                'name' => 'Measurement Assign',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{responsible_user}', '{customer_name}', '{customer_email}', '{customer_phone_number}', '{measurement_number}', '{date}', '{cloth_type}', '{measurement_details}'],
                'subject' => 'New Measurement Assigned - {measurement_number}',
                'templete' => '<p><strong>Dear {responsible_user},</strong></p><p>A new measurement has been assigned to you. Please find the details below:</p><ul><li><strong>Measurement Number:</strong> {measurement_number}</li><li><strong>Date:</strong> {date}</li><li><strong>Cloth Type:</strong> {cloth_type}</li><li><strong>Measurement Details:</strong>&nbsp;</li></ul><p>{measurement_details}</p><p><strong>Customer Information:</strong></p><ul><li>Name: {customer_name}</li><li>Email: {customer_email}</li><li>Phone: {customer_phone_number}</li></ul><p>If you have any questions, please contact us at:</p><ul><li>Email: <a href="mailto:{company_email}">{company_email}</a></li><li>Phone: {company_phone_number}</li></ul><p>Thank you, <strong>{company_name}</strong>, {company_address}, {company_currency}</p>',
                'sms_message' => 'Hi {customer_name}, your measurement #{measurement_number} on {date} has been recorded at {company_name}. Cloth: {cloth_type}.'
            ],
            'order_create' => [
                'module' => 'order_create',
                'name' => 'New Order',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{order_number}', '{order_date}', '{deadline_date}', '{gemder}', '{cloth_type}', '{quantity}', '{febric}', '{febric_color}', '{responsible_user_name}', '{responsible_user_email}', '{responsible_user_phone}', '{order_status}', '{measurement_details}'],
                'subject' => 'Order Confirmation - {order_number}',
                'templete' => '<p><strong>Dear {customer_name},</strong></p><p>Thank you for placing your order with <strong>{company_name}</strong>! We are pleased to confirm your order. Below are the details of your order:</p><ul><li><strong>Order Number:</strong> {order_number}</li><li><strong>Order Date:</strong> {order_date}</li><li><strong>Cloth Type:</strong> {cloth_type}</li><li><strong>Quantity:</strong> {quantity}</li><li><strong>Fabric:</strong> {febric}</li><li><strong>Fabric Color:</strong> {febric_color}</li><li><strong>Measurement Details:</strong>&nbsp;</li></ul><p>{measurement_details}</p><ul><li><strong>Order Status:</strong> {order_status}</li></ul><p>If you have any questions or need assistance, you can contact us:</p><ul><li><strong>Responsible User:</strong> {responsible_user_name}</li><li><strong>Email:</strong> <a href="mailto:{responsible_user_email}">{responsible_user_email}</a></li><li><strong>Phone:</strong> {responsible_user_phone}</li></ul><p>Thank you for choosing <strong>{company_name}</strong>. We look forward to serving you again!</p><p><strong>{company_name}</strong><br>{company_address}<br>{company_currency}</p>',
                'sms_message' => 'Hi {customer_name}, your order {order_number} on {order_date} has been confirmed at {company_name}. Status: {order_status}.',
            ],
            'order_assign' => [
                'module' => 'order_assign',
                'name' => 'Order Assign',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{responsible_user}', '{customer_name}', '{customer_email}', '{customer_phone_number}', '{order_number}', '{order_date}', '{deadline_date}', '{gemder}', '{cloth_type}', '{quantity}', '{febric}', '{febric_color}', '{order_status}', '{measurement_details}'],
                'subject' => 'New Order Assigned - {order_number}',
                'templete' => '<p><strong>Dear {responsible_user},</strong></p><p>You have been assigned a new order. Please find the details below:</p><ul><li><strong>Order Number:</strong> {order_number}</li><li><strong>Order Date:</strong> {order_date}</li><li><strong>Deadline Date:</strong> {deadline_date}</li><li><strong>Customer Name:</strong> {customer_name}</li><li><strong>Customer Email:</strong> {customer_email}</li><li><strong>Customer Phone:</strong> {customer_phone_number}</li><li><strong>Cloth Type:</strong> {cloth_type}</li><li><strong>Quantity:</strong> {quantity}</li><li><strong>Fabric:</strong> {febric}</li><li><strong>Fabric Color:</strong> {febric_color}</li><li><strong>Order Status:</strong> {order_status}</li><li><strong>Measurement Details:</strong>&nbsp;</li><li>{measurement_details}</li></ul><p>If you have any questions or require additional information, feel free to contact us at:</p><ul><li>Email: <a href="mailto:{company_email}">{company_email}</a></li><li>Phone: {company_phone_number}</li></ul><p>Thank you, <strong>{company_name}</strong>, {company_address}, {company_currency}</p>',
                'sms_message' => 'Hi {customer_name}, your order {order_number} on {order_date} has been confirmed at {company_name}. Status: {order_status}.',
            ],
            'order_status_update' => [
                'module' => 'order_status_update',
                'name' => 'Order Status Update',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{order_number}', '{order_date}', '{deadline_date}', '{order_status}'],
                'subject' => 'Order Status Update - {order_number}',
                'templete' => '<p><strong>Dear {customer_name},</strong></p>
                    <p>Your order status has been updated. Please find the latest details below:</p>
                    <ul>
                    ...
                    <ul>
                        ...
                        <li>Phone: {company_phone_number}</li>
                    </ul>
                    <p>Thank you,<br><strong>{company_name}</strong><br>{company_address}, {company_currency}</p>',
                'sms_message' => 'Hi {customer_name}, your order {order_number} placed on {order_date} has been updated. New Status: {order_status}. - {company_name}',
            ],

            'invoice_create' => [
                'module' => 'invoice_create',
                'name' => 'New invoice',
                'short_code' => ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{invoice_number}', '{cloth_type}', '{invoice_amount}', '{invoice_date}', '{due_date}'],
                'subject' => 'New Invoice from {company_name}',
                'templete' => '<p><strong>Dear {customer_name},</strong></p><p>We are pleased to inform you that your invoice has been successfully generated. Please find the details below:</p><ul><li><strong>Invoice Number:</strong> {invoice_number}</li><li><strong>Order / Cloth Type:</strong> {cloth_type}</li><li><strong>Invoice Date:</strong> {invoice_date}</li><li><strong>Invoice Amount:</strong> {company_currency} {invoice_amount}</li><li><strong>Due Date:</strong> {due_date}</li></ul><p>If you have any questions or need further assistance, please contact us:</p><ul><li>Email: <a href="mailto:{company_email}">{company_email}</a></li><li>Phone: {company_phone_number}</li></ul><p>Thank you for your trust in <strong>{company_name}</strong>. We look forward to serving you again!</p><p><strong>{company_name}</strong><br>{company_address}</p>',
                'sms_message' => 'Hi {customer_name}, invoice {invoice_number} for {cloth_type} is generated: {company_currency}{invoice_amount}, due {due_date}. - {company_name}',
            ],
        ];
    }
}
    ...
    function MessageReplace($notification, $id = 0)
    {
        ...
        if (!empty($notification)) {
            ...
            if ($notification->module == 'customer_create') {
                $user = User::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $user->name, env('APP_URL'), $user->email, $notification['password']];
            }
            if ($notification->module == 'measurement_create') {
                $measuremet = Measurement::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{measurement_number}', '{date}', '{cloth_type}', '{responsible_user_name}', '{responsible_user_email}', '{responsible_user_phone}', '{measurement_details}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $measuremet->customers->name, measurementPrefix() . $measuremet->measurement_id, dateFormat($measuremet->date), $measuremet->clothTypes->title, $measuremet->users->name, $measuremet->users->email, $measuremet->users->phone_number, $notification->measurement];
            }
            if ($notification->module == 'measurement_assign') {
                $measuremet = Measurement::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{responsible_user}', '{customer_name}', '{customer_email}', '{customer_phone_number}', '{measurement_number}', '{date}', '{cloth_type}', '{measurement_details}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $measuremet->users->name, $measuremet->customers->name, $measuremet->customers->email, $measuremet->customers->phone_number, measurementPrefix() . $measuremet->measurement_id, dateFormat($measuremet->date), $measuremet->clothTypes->title, $notification->measurement];
            }
            if ($notification->module == 'order_create') {
                $order = Order::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{order_number}', '{order_date}', '{deadline_date}', '{gemder}', '{cloth_type}', '{quantity}', '{febric}', '{febric_color}', '{responsible_user_name}', '{responsible_user_email}', '{responsible_user_phone}', '{order_status}', '{measurement_details}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $order->customers->name, orderPrefix() . $order->order_id, dateFormat($order->order_date), dateFormat($order->deadline_date), $order->gender, $order->clothTypes->title, $order->quantity, $order->febric, $order->febric_color, $order->users->name, $order->users->email, $order->users->phone_number, $order->status, $notification->measurement];
            }
            if ($notification->module == 'order_assign') {
                $order = Order::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{responsible_user}', '{customer_name}', '{customer_email}', '{customer_phone_number}', '{order_number}', '{order_date}', '{deadline_date}', '{gemder}', '{cloth_type}', '{quantity}', '{febric}', '{febric_color}', '{order_status}', '{measurement_details}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $order->users->name, $order->customers->name, $order->customers->email, $order->customers->phone_number, orderPrefix() . $order->order_id, dateFormat($order->order_date), dateFormat($order->deadline_date), $order->gender, $order->clothTypes->title, $order->quantity, $order->febric, $order->febric_color, $order->status, $notification->measurement];
            }
            if ($notification->module == 'order_status_update') {
                $order = Order::find($id);
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{order_number}', '{order_date}', '{deadline_date}', '{order_status}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $order->customers->name, orderPrefix() . $order->order_id, dateFormat($order->order_date), dateFormat($order->deadline_date), $order->status];
            }
            if ($notification->module == 'invoice_create') {
                $invoice = Invoice::find($id);
                $invoiceItem = InvoiceItem::where('invoice_id', $invoice->id)->first();
                $search = ['{company_name}', '{company_email}', '{company_phone_number}', '{company_address}', '{company_currency}', '{customer_name}', '{invoice_number}', '{cloth_type}', '{invoice_amount}', '{invoice_date}', '{due_date}'];
                $replace = [$settings['company_name'], $settings['company_email'], $settings['company_phone'], $settings['company_address'], $settings['CURRENCY_SYMBOL'], $invoice->customers->name, invoicePrefix() . $invoice->invoice_id, $invoiceItem->clothTypes->title, $invoiceItem->amount, dateFormat($invoice->invoice_date), dateFormat($invoice->due_date)];
            }
    ...
    function HomePageSection()
    {
        $retuen = [
            ...
            [
                'title' => 'Banner',
                'section' => 'Section 1',
                'content_value' => '{"name":"Banner","section_enabled":"active","title":"Smart TMS SaaS - Tailor Management System","sub_title":"Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. This innovative platform streamlines the entire tailoring process, from customer management and order tracking to inventory control and financial reporting, enabling tailors to focus on their craftsmanship while enhancing operational efficiency.","btn_name":"Get Started","btn_link":"#","section_footer_text":"Manage your business efficiently with our all-in-one solution designed for performance, security, and scalability.","section_footer_image":{},"section_main_image":{},"section_footer_image_path":"upload\/homepage\/banner_2.png","section_main_image_path":"upload\/homepage\/banner_1.png","box_image_1_path":"","box_image_2_path":"","box_image_3_path":"","Box1_image_path":"","Box2_image_path":"","Sec4_box1_image_path":"","Sec4_box2_image_path":"","Sec4_box3_image_path":"","Sec4_box4_image_path":"","Sec4_box5_image_path":"","Sec4_box6_image_path":"","Sec7_box1_image_path":"","Sec7_box2_image_path":"","Sec7_box3_image_path":"","Sec7_box4_image_path":"","Sec7_box5_image_path":"","Sec7_box6_image_path":"","Sec7_box7_image_path":"","Sec7_box8_image_path":""}',
                'content' => '
                <header id="home">
                    <div class="container">
                        <div class="row align-items-center justify-content-between">
                            <div class="col-lg-5 col-xl-4">
                                <h1 class="mt-sm-3 mb-sm-4 f-w-600 wow fadeInUp" data-wow-delay="0.2s">
                                    ...
                                    <span class="text-primary">Smart TMS SaaS - Tailor Management System</span>
                                </h1>
                                <h4 class="mb-sm-4 text-muted wow fadeInUp" data-wow-delay="0.4s">
                                    Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. This innovative platform streamlines the entire tailoring process, from customer management and order tracking to inventory control and financial reporting, enabling tailors to focus on their craftsmanship while enhancing operational efficiency.
                                </h4>
                        ...
                        <div class="container">
                            <div class="row align-items-center g-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <p class="text-lg w-75 my-3 my-md-4">Smart TMS has made it easy for developers of any skill level to
                                        use their product.</p>
                    ...
                    <div class="container">
                        <div class="row justify-content-center title">
                            <div class="col-md-9 col-lg-6 text-center">
                                <h2 class="h1">What does Smart TMS offer?</h2>
                                <p class="text-lg">
                                    Smart TMS is a reliable choice for your admin panel needs, offering a wide range of features to
                        ...
                        <div class="row g-4 text-center">
                            <div class="col-md-6 col-xl-4">
                                ...
                                <p>
                                    Smart TMS can improve the user experience of your web application by providing a clear and intuitive
                            ...
                            <div class="col-md-6 col-xl-4">
                                ...
                                <p>
                                    Smart TMS can save developers time and effort by providing a pre-built user interface, allowing them
                            ...
                            <div class="col-md-6 col-xl-4">
                                ...
                                <p>Smart TMS simplifies admin panel development with easy theme setup and clear code with flexible
                                    layouts options.</p>
                            </div>
                            <div class="col-md-6 col-xl-4">
                                ...
                                <p>
                                    Smart TMS uses scalable technologies and resources to ensure that your admin panel remains efficient
                            ...
                            <div class="col-md-6 col-xl-4">
                                ...
                                <p>
                                    ...
                                    and effectively use the Smart TMS.
                                </p>
                            </div>
                            <div class="col-md-6 col-xl-4">
                                <img src="assets/images/landing/img-design-6.svg" alt="img" class="img-fluid" />
                                <h3 class="my-4 fw-semibold">Performance Centric</h3>
                                <p>Smart TMS is a performance-centric dashboard template that is designed to deliver optimal performance
                    ...
                    <div class="container">
                        <div class="row justify-content-center title">
                            <div class="col-md-9 col-lg-6 text-center">
                                ...
                                <p class="text-lg">Smart TMS has conceptul working apps like Chat, Inbox, E-commerce, Invoice,
            ...
            [
                'title' => 'Core Features',
                'section' => 'Section 6',
                'content_value' => '{"name":"Core Features","section_enabled":"active","Sec6_title":"Core Features","Sec6_info":"Core Modules For Your Business","Sec6_Box_title":["Dashboard","Cloth Type","Measurement","Order","Invoice"],"Sec6_Box_subtitle":["Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. ","Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. ","Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. ","Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. ","Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. ","Complete Message App with all possible option"],"Sec6_box_image":[{},{},{},{},{},{}],"section_footer_image_path":"","section_main_image_path":"","box_image_1_path":"","box_image_2_path":"","box_image_3_path":"","Box1_image_path":"","Box2_image_path":"","Sec4_box1_image_path":"","Sec4_box2_image_path":"","Sec4_box3_image_path":"","Sec4_box4_image_path":"","Sec4_box5_image_path":"","Sec4_box6_image_path":"","Sec6_box0_image_path":"upload\/homepage\/1.png","Sec6_box1_image_path":"upload\/homepage\/2.png","Sec6_box2_image_path":"upload\/homepage\/3.png","Sec6_box3_image_path":"upload\/homepage\/4.png","Sec6_box4_image_path":"upload\/homepage\/5.png","Sec6_box5_image_path":"upload\/homepage\/6.png","Sec6_box6_image_path":"","Sec7_box1_image_path":"","Sec7_box2_image_path":"","Sec7_box3_image_path":"","Sec7_box4_image_path":"","Sec7_box5_image_path":"","Sec7_box6_image_path":"","Sec7_box7_image_path":"","Sec7_box8_image_path":""}',
                'content' => '
                    ...
                    <div class="container">
                        <div class="row justify-content-center title">
                            <div class="col-md-9 col-lg-6 text-center">
                                ...
                                <p class="text-lg">Smart TMS has conceptul working apps like Chat, Inbox, E-commerce, Invoice, Kanban,
                                    and Calendar</p>
                            </div>
                        </div>
                        <div class="row text-center justify-content-center">
                    ...
                    <div class="container">
                        <div class="row align-items-center">
                            <div class="col-md-9">
                                <div class="d-flex align-items-center">
                                    <div class="flex-shrink-0">
                                        <h2 class="mb-0 text-white">Choose Smart TMS for</h2>
                                    </div>
                                    <div class="flex-grow-1 ms-3">
                            ...
                            <div class="col-12">
                                <div class="accordion accordion-flush" id="accordionFlushExample">
                                    ...
                                    <h2 class="accordion-header" id="flush-headingThree">
                                        ...
                                        <b>Is Smart TMS Support Typescript?</b>
                                        </button>
                                    </h2>
                                    <div id="flush-collapseThree" class="accordion-collapse collapse" aria-labelledby="flush-headingThree"
                                        data-bs-parent="#accordionFlushExample">
                                        <div class="accordion-body text-muted">
                                        Yes, Smart TMS Support the TypeScript and it is only available in Plus and Extended License.
                                        </div>
                                    </div>
                                    </div>
                                    <div class="accordion-item">
                                    <h2 class="accordion-header" id="flush-headingfour">
                                        ...
                                        <b>Is there any Road map for Smart TMS?</b>
                                        </button>
                                    </h2>
                                    <div id="flush-collapse-four" class="accordion-collapse collapse" aria-labelledby="flush-headingfour"
                                        data-bs-parent="#accordionFlushExample">
                                        <div class="accordion-body text-muted">
                                        Smart TMS is our flagship React Dashboard Template and we always add the new features for the long
                                        run. You can check
            ...
            [
                ...
                'section' => 'Section 10',
                'content_value' => '{"name":"AboutUS - Footer","section_enabled":"active","Sec10_title":"About Smart TMS SaaS","Sec10_info":"Smart TMS is a comprehensive Software as a Service (SaaS) solution designed specifically for the tailoring and bespoke garment industry. This innovative platform streamlines the entire tailoring process, from customer management and order tracking to inventory control and financial reporting, enabling tailors to focus on their craftsmanship while enhancing operational efficiency.","section_footer_image_path":"","section_main_image_path":"","box_image_1_path":"","box_image_2_path":"","box_image_3_path":"","Box1_image_path":"","Box2_image_path":"","Sec4_box1_image_path":"","Sec4_box2_image_path":"","Sec4_box3_image_path":"","Sec4_box4_image_path":"","Sec4_box5_image_path":"","Sec4_box6_image_path":"","Sec7_box1_image_path":"","Sec7_box2_image_path":"","Sec7_box3_image_path":"","Sec7_box4_image_path":"","Sec7_box5_image_path":"","Sec7_box6_image_path":"","Sec7_box7_image_path":"","Sec7_box8_image_path":""}',
                'content' => '
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-md-4 wow fadeInUp" data-wow-delay="0.2s">
                                ...
                                <h4 class="my-3 text-white">About Smart TMS</h4>
                            ...
                            <div class="col-md-8">
                                <div class="row g-4">
                                    ...
                                    <div class="col-6 col-md-3 wow fadeInUp" data-wow-delay="0.6s">
                                        <h5 class="mb-3 mb-sm-4 text-white">Smart TMS Eco-System</h5>
                                        <ul class="list-unstyled footer-link">
...
if (!function_exists('authPage')) {
    ...
    if (!function_exists('NewPermission')) {
        function NewPermission()
        {
            ...
            $permissions = [
                // Version : 1.4 New Permission
                ['name' => 'manage twilio settings', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'manage payment settings', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'manage invoice', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'show invoice', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'create measurement', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'manage measurement', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'show measurement', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'edit measurement', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'delete measurement', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'manage customer', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'create customer', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'edit customer', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'show customer', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'delete customer', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'manage contact', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'create contact', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'edit contact', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'delete contact', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'manage note', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'manage order', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'create order', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'edit order', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'delete order', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'manage order calendar', 'guard_name' => 'web', 'roles' => ['employee']],
                ['name' => 'show order', 'guard_name' => 'web', 'roles' => ['customer', 'employee']],
                ['name' => 'manage expense category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'craete expense category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'edit expense category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'delete expense category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'manage expense sub category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'craete expense sub category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'edit expense sub category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'delete expense sub category', 'guard_name' => 'web', 'roles' => ['owner', 'manager']],
                ['name' => 'manage order report', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'manage income report', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'manage expense report', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'manage profit loss report', 'guard_name' => 'web', 'roles' => ['owner']],
                ['name' => 'update order status', 'guard_name' => 'web', 'roles' => ['owner']],
            ];
            ...
            // Version : 1.4 Remove Permission
            $removePermissions = [
                ['name' => 'create note', 'guard_name' => 'web', 'roles' => ['manager']],
                ['name' => 'edit note', 'guard_name' => 'web', 'roles' => ['manager']],
                ['name' => 'delete note', 'guard_name' => 'web', 'roles' => ['manager']],
                ['name' => 'show note', 'guard_name' => 'web', 'roles' => ['manager']],
            ];
