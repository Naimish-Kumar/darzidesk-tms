...
@push('css-page')
    <style>
        ...
        .table-custom-fin td .font-weight-bold,
        .table-custom-fin td div,
        .table-custom-fin td span {
            color: #F8FAFC !important;
        }

        .table-custom-fin tbody tr:hover td {
            background-color: rgba(217, 164, 65, 0.08) !important;
        }
    </style>
@endpush
...
@section('content')
    ...
    <div class="fin-header-bar d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h4 class="mb-0 font-weight-bold text-white">{{ __('Financial & Revenue Intelligence') }}</h4>
                <small style="color: #CBD5E1;">{{ __('Real-time financial performance, profit margins, payouts and asset valuations') }}</small>
            </div>
        </div>

        <!-- Filter Selector -->
        <form method="GET" action="{{ route('financials.analytics') }}" class="d-flex align-items-center gap-2">
            <label class="fs-8 font-weight-bold mb-0 text-uppercase" style="color: #CBD5E1;">{{ __('PERIOD') }}:</label>
            <select name="filter" onchange="this.form.submit()" class="form-select form-select-sm rounded-3 font-weight-bold" style="width: 160px; background-color: #102B45; border-color: #29435D; color: #FFFFFF;">
    ...
    <div class="net-profit-banner d-flex flex-wrap align-items-center justify-content-between gap-4">
        <div>
            ...
            <p class="mb-0 opacity-75 fs-7" style="color: #CBD5E1;">{{ __('Calculated as Total Collections minus Operating Expenses & Tailor Payouts') }}</p>
        </div>
        <div class="d-flex gap-4">
            <div class="border-end border-white-50 pe-4">
                <small class="d-block font-weight-bold text-uppercase fs-8" style="color: #CBD5E1;">{{ __('PROFIT MARGIN') }}</small>
                <span class="fs-3 font-weight-bold text-white">{{ number_format($profitMargin, 1) }}%</span>
            </div>
            <div>
                <small class="d-block font-weight-bold text-uppercase fs-8" style="color: #CBD5E1;">{{ __('TOTAL COSTS') }}</small>
    ...
    <div class="row g-4 mb-4">
        <div class="col-lg-8 col-12">
            <div class="chart-card-custom">
                <div class="d-flex align-items-center justify-content-between pb-3 mb-3 border-bottom" style="border-color: #29435D !important;">
                    <div>
                        <h5 class="mb-0 font-weight-bold text-white"><i class="ti ti-trending-up me-2" style="color: var(--dd-gold);"></i>{{ __('Revenue vs. Operating Costs Trend') }}</h5>
                        <small style="color: #CBD5E1;">{{ __('Monthly comparison over the last 6 months') }}</small>
                    </div>
                </div>
        ...
        <div class="col-lg-4 col-12">
            <div class="chart-card-custom">
                <div class="d-flex align-items-center justify-content-between pb-3 mb-3 border-bottom" style="border-color: #29435D !important;">
                    <div>
                        <h5 class="mb-0 font-weight-bold text-white"><i class="ti ti-credit-card me-2" style="color: var(--dd-gold);"></i>{{ __('Payment Methods') }}</h5>
                        <small style="color: #CBD5E1;">{{ __('Collections by payment mode') }}</small>
                    </div>
                </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4 col-12">
            <div class="chart-card-custom">
                <div class="d-flex align-items-center justify-content-between pb-3 mb-3 border-bottom" style="border-color: #29435D !important;">
                    <h5 class="mb-0 font-weight-bold text-white"><i class="ti ti-category text-danger me-2"></i>{{ __('Expense Categories') }}</h5>
                </div>
                <div>
                    @forelse($expenseCategories as $cat)
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-between mb-1">
                                <span class="font-weight-bold fs-7 text-white">{{ $cat->category->name ?? 'General Expense' }}</span>
                                <span class="font-monospace fs-7" style="color: #CBD5E1;">{{ $currency . number_format($cat->total, 2) }} ({{ number_format($pct, 1) }}%)</span>
                            </div>
                            <div class="progress" style="height: 8px; border-radius: 4px; background-color: #102B45;">
                ...
                <div class="mt-4 p-3 rounded-3" style="background: var(--dd-gold-light); border: 1px solid rgba(217, 164, 65, 0.3);">
                    ...
                    <div class="h4 font-weight-bold mb-0 text-white">{{ $currency . number_format($stockValuation, 2) }}</div>
                    <small class="fs-8 d-block mt-1" style="color: #CBD5E1;">{{ __('Valuation of raw fabric & trims across workshops') }}</small>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8 col-12">
            <div class="chart-card-custom">
                <div class="d-flex align-items-center justify-content-between pb-3 mb-3 border-bottom" style="border-color: #29435D !important;">
                    <div>
                        <h5 class="mb-0 font-weight-bold text-white"><i class="ti ti-history text-success me-2"></i>{{ __('Recent Payment Transactions') }}</h5>
                        <small style="color: #CBD5E1;">{{ __('Latest incoming collections and advance payments') }}</small>
                    </div>
                </div>
                            ...
                            @forelse($recentPayments as $payment)
                                <tr>
                                    <td>
                                        ...
                                        @else
                                            <span class="font-monospace" style="color: #CBD5E1;">#INV</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="badge px-2 py-1" style="background: #102B45; color: #FFFFFF; border: 1px solid #29435D;">
                                            <i class="ti ti-credit-card me-1"></i>{{ $payment->payment_type ?: 'Cash' }}
                                        </span>
                                    </td>
                                    <td><span style="color: #CBD5E1;">{{ dateFormat($payment->payment_date ?: $payment->created_at) }}</span></td>
